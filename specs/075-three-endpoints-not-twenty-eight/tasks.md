# Tasks — Spec 075, three endpoints, not twenty-eight

**Phase:** 3 (Tasks) · **Spec:** `spec.md` · **Plan:** `plan.md` · **Issue:** #2096

**Engineer:** `backend-engineer` throughout. One C# guard in
`tests/Architecture.Tests` plus three fluent lines and two comment corrections
in three `src/*/Api` files. No frontend, no infrastructure, no Aspire wiring, no
...
honest. T007–T009 are a real fan-out: three disjoint `*Endpoints.cs` files in two
contexts, no shared state, no ordering between them.
...
- **[T001] [US-1]** Create
  ...
  class-level XML doc that states, in this order: the claim; that the two sets
  are **typed in, not derived**; the question a new endpoint's author answers to
  join one (*does its command handler mutate or delete an aggregate that already
  exists?*); and the five things the guard does not prove (spec, *What the guard
  cannot do*). Add the readers: `RepositoryRoot()`, `/`-normalisation, `\r`
  stripping, and the `src/*/Api/**/*.cs` glob.
...
- **[T004] [US-1]** The two pinned sets and the partition assertion A-2 (FR-003,
  FR-007): **28** routes that declare `Status409Conflict` and **5** that must
  not — `POST /rules/{name}/dry-run`, `POST /events/manual`,
  `POST /events/webhook/{integrationName}`, `POST /streams/authorize`,
  `POST /streams/kiosk-latency`. Each of the five carries a one-line reason in a
  comment beside it. A route in neither set fails as *unclassified* (FR-005).
  *Depends on: T003. Blocks: T005.*

  **Done when:** the pinned 28 includes the three that do not yet declare it —
  `POST /cameras/{camera:guid}/retire`, `DELETE /devices/{clientId}`,
  `DELETE /kiosks/{clientId}` — so the assertion is **red** on unmodified `src/`.
  Writing the sets from today's source instead is the failure T006 exists to
  catch.
...
- **[T006] [US-1]** Run `dotnet test tests/Architecture.Tests` against
  ...
  **Done when** all four hold on the same run:
  - A-2 is **red naming exactly three routes**: `POST /cameras/{camera:guid}/retire`,
    `DELETE /devices/{clientId}`, `DELETE /kiosks/{clientId}`.
  - **No fourth route** appears — a fourth means a chain was terminated early.
  - The **five** are reported as correctly non-declaring (mirror green).
...
## The declarations — three disjoint files, real fan-out
...
- **[T010] [US-1]** Run `dotnet test tests/Architecture.Tests` again, plus
  `dotnet build -c Release` for the three edited projects and the test project.
  *Depends on: T007, T008, T009. Blocks: T011.*
...
- **[T011] [US-1]** Confirm the diff's shape (spec, procedure step 6): `git diff
  src/` contains only three added `.ProducesProblem` lines and comment text — no
  `Produces<T>`, no route, no `RequireAuthorization`, no handler body, no
  `WithSummary`. Re-run the census commands and record **28** declaring rows.
...
- Adding 409 to the five endpoints that cannot produce one. That is the defect,
