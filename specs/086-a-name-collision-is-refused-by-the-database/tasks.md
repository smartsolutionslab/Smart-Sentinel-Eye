...
| **T003** | | **Red concurrency integration test** in `tests/Integration.Tests/OverlayDesigner/` — two `POST /overlays` with an identical name issued from two tasks started together; assert exactly one `201`, one `409`, and `count(*) = 1`. **Must be observed failing against the current schema** (both requests answer `201`) and the failure quoted in the PR. A test first run after the index exists proves the index is present, not that it changed anything. **Observed (phase 4b, recorded phase 6): it did not fail.** Twelve unawaited writers were dispatched twice on a clean box; the handler's own check won every time. The test was green before the change and green after; it adds **no** phase-4a evidence and becomes an invariant once the index exists, like `UniquenessRaceIntegrationTests`. The red that discharges this story's phase-4a obligation is the schema-level test in the same file, `A_second_live_chain_with_the_same_name_is_refused_by_the_database`, which fails deterministically. | T001 |
...
| **T012** | [P] | **Red concurrency integration test** in `tests/Integration.Tests/LayoutComposition/` — two `POST /layouts` with an identical name **in one fab**, concurrent; one `201`, one `409`, `count(*) = 1`. Observed failing on the current schema; failure quoted. **Observed (phase 4b, recorded phase 6): it did not fail** — same outcome and same reasoning as T003, and the same substitute red: `A_second_live_chain_with_the_same_name_in_one_fab_is_refused_by_the_database`. | T010 |
