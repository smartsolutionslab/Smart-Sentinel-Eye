...
| **T-004** | | US-1 | **Observe and quote the red.** Re-run the T-001 command. Expect **exactly two** new failures — `WhepValidatorIssuerTests` case 1 and the T-003 parity case — and the other seven green. Capture the verbatim output; it is the phase-4 gate evidence and goes in the PR body (ADR-0139). If any other test is red, stop: the harness is wrong, not the code. **Commit here** (`test(089): …`). |
...
| **T-007** | | US-1 | **Green, and the counterfactual.** Re-run the T-001 command: nine green. Then prove each guard by constructing what it claims to catch (both reverted immediately): re-add `ValidIssuer = authority` → case 1 fails; set `ValidateIssuer = false` → case 2 fails. Two different weakenings, two different failures. Record both in the PR. Also check the Release build for SonarAnalyzer S1541/S138 on `ValidateAsync` (now 33 physical lines) — **only if it actually fires**, extract the claim-reading tail at `:68-77` into `private static Option<WhepAuthSubject> SubjectFrom(ClaimsPrincipal principal)`. Do not pre-emptively refactor. **Commit here** (`fix(089): …`). |
